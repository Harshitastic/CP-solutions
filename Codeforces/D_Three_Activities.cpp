...
#define invl(v,n) vector<int> v(n);for(ll i=0;i<n;i++){cin>>v[i];}
...
/*--------------------------------------------------------------------------------------------------------------------------------*/
// Solve function
vector<int> max3(vector<int>&arr){
    ...
    _sort(temp);
    _reverse(temp);
    return {temp[0].second,temp[1].second,temp[2].second};
}
void solve(){
    ll t;
    cin>>t;
    while(t--){
            ...
            invl(b,n)
            invl(c,n)
            vector<int> maxx1=max3(a);
            vector<int> maxx2=max3(b);
            vector<int> maxx3=max3(c);
            int ans=0;
            for(int i=0;i<3;i++){
                for(int j=0;j<3;j++){
                    for(int k=0;k<3;k++){
                        int x=maxx1[i],y=maxx2[j],z=maxx3[k];
                        if((x==y) or (y==z) or (z==x)) C;
